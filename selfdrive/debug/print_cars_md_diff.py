...
CARS_MD = os.path.join(BASEDIR, "docs", "CARS.md")
...
def print_car_docs_diff(path):
  with open(path, 'r') as f1, open(CARS_MD, 'r') as f2:
    base_cars_md = f1.readlines()
    new_cars_md = f2.readlines()

    d = difflib.Differ()
    diff = list(d.compare(base_cars_md, new_cars_md))

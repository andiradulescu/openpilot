...
class Controls:
  def __init__(self, sm=None, pm=None, can_sock=None, CI=None):
    ...
    self.dynamic_longitudinal_control = False
      ...
      if self.events.any(ET.USER_DISABLE):
        ...
        if self.state == State.enabled:
          if CS.cruiseState.enabled and not self.CS_prev.cruiseState.enabled:
            self.v_cruise_helper.initialize_v_cruise(CS, self.experimental_mode, self.is_metric, self.dynamic_longitudinal_control)
          # Block resume if cruise never previously enabled
          resume_pressed = any(be.type in (ButtonType.accelCruise, ButtonType.resumeCruise) for be in CS.buttonEvents)
          if not self.CP.pcmCruise and not self.v_cruise_helper.v_cruise_initialized and resume_pressed:
  ...
  def step(self):
    ...
    self.dynamic_longitudinal_control = self.params.get_bool("DynamicLongitudinalControl")

...
class TestParseStatus:
  ...
  def test_ssid_decoded(self):
    ...
    d = parse_status(raw)
    assert d["ssid"] == "café"
    assert d["ssid"].encode("utf-8") == b"caf\xc3\xa9"  # codespell:ignore caf
...
class TestDecodeSsid:
  ...
  def test_utf8_multibyte(self):
    ...
    decoded = decode_ssid("caf\\xc3\\xa9")  # codespell:ignore caf
    assert decoded == "café"
...
class TestNetworkManagerCompatibility:
  def test_unmanage_skips_when_nmcli_is_absent(self, mocker):
    mocker.patch.object(wpa_ctrl_module.shutil, "which", return_value=None)
    run = mocker.patch.object(wpa_ctrl_module.subprocess, "run")

    assert wpa_ctrl_module._unmanage_wlan0()
    run.assert_not_called()

  def test_unmanage_uses_discovered_nmcli(self, mocker):
    mocker.patch.object(wpa_ctrl_module.shutil, "which", return_value="/usr/bin/nmcli")
    run = mocker.patch.object(wpa_ctrl_module.subprocess, "run")
  ...
  def test_unmanage_failure_is_nonfatal(self, mocker):
    mocker.patch.object(wpa_ctrl_module.shutil, "which", return_value="/usr/bin/nmcli")
    run = mocker.patch.object(wpa_ctrl_module.subprocess, "run")
...
class TestTetheringDnsmasqOwnership:
  def test_stop_targets_only_openpilot_tethering(self, mocker):
    run = mocker.patch.object(wpa_ctrl_module.subprocess, "run")

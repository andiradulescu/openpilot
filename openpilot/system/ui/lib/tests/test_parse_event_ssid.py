...
class TestParseEventSsid:
  ...
  def test_hex_escape(self):
    ...
    event = 'CTRL-EVENT-SSID-TEMP-DISABLED id=0 ssid="caf\\xc3\\xa9" reason=WRONG_KEY'  # codespell:ignore caf
    ssid = parse_event_ssid(event)
    assert ssid == "café"
    assert ssid.encode("utf-8") == b"caf\xc3\xa9"  # codespell:ignore caf

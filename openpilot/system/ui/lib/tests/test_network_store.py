...
from openpilot.system.ui.lib.wpa_ctrl import _generate_wpa_conf
...
class TestNetworkStore(TestCase):
  ...
  def test_loads_autoconnect_priority(self):
    ...
    raw = Path(self.persistent, "preferred-uuid-Preferred.nmconnection").read_text()
  ...
  def test_preserves_bssid_restriction(self):
    ...
    raw = Path(self.persistent, "pinned-uuid-Pinned.nmconnection").read_text()
  ...
  def test_preserves_non_utf8_decimal_list_ssid(self):
    ...
    _generate_wpa_conf(store, config_path)
  ...
  def test_edit_persistent_profile_removes_shadowed_runtime_copy(self):
    ...
    netplan_path = Path(self.netplan, "90-NM-shared-uuid.yaml")
  ...
  def test_metered_updates_every_profile_with_the_same_ssid(self):
    ...
    for file_uuid in ("first-uuid", "second-uuid"):
      raw = Path(self.persistent, f"{file_uuid}-Duplicate.nmconnection").read_text()
      assert "metered = 1" in raw

  def test_replacement_psk_updates_every_profile_with_the_same_ssid(self):
    write_profile(
      self.persistent,
      "first.nmconnection",
      "Duplicate",
  ...
  def test_forget_duplicate_runtime_profiles_removes_every_netplan_source(self):
    ...
    netplan_paths = {
      Path(self.netplan, "90-NM-first-uuid.yaml"),
      Path(self.netplan, "90-NM-second-uuid.yaml"),
    }
  ...
  def test_accepts_only_legacy_dns_priority(self):
    legacy_path = write_profile(self.persistent, "legacy.nmconnection", "Legacy")
    custom_path = write_profile(self.persistent, "custom.nmconnection", "Custom")
    for path, priority in ((legacy_path, 600), (custom_path, 100)):
...
""")
    ...
    assert require_entry(store, "Legacy")["_ipv4"]["dns-priority"] == "600"
